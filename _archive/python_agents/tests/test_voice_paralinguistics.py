...
async def test_voice_agent_ingests_paralinguistic_tags(voice_agent):
    ...
    with patch("app.voice.agent.vad_to_prosody", return_value={"rate": 1.0, "pitch": 1.0, "volume": 1.0}):
...
async def test_rendering_paralinguistic_tag_calls_filler_service(voice_agent):
    # Mock filler service
    voice_agent.filler_service.get_specific_filler = MagicMock(return_value=b"fake_pcm_data")
