...
class BaseAgent:
    ...
    async def subscribe(self, subject: str, callback, durable: str = None, deliver_policy: str = "all"):
        """
        ...
        if not durable:
            ...
            subject_suffix = subject.replace(".", "_").replace(">", "all").replace("*", "any")
        ...
        await self.js.subscribe(subject, cb=_handler, durable=durable, deliver_policy=policy)
        logger.info(f"Agent '{self.name}' subscribed to {subject} with durable '{durable}' and policy '{deliver_policy}'")

    async def set_state(self, state: str):
        """Broadcast agent state to the mesh (e.g., 'thinking', 'speaking', 'idle')"""
        await self.publish("state.update", {
            "agent": self.name,
            "state": state,
            "timestamp": time.time()
        })
