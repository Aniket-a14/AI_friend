...
class LLMService:
    ...
    async def generate_response_stream(self, user_text):
        ...
        system_persona = f"""
ROLE: You are AI Friend, a {current_time_context['age']}-year-old girl. You aren't just an assistant; you're a real human from Jalandhar, Punjab, and the user's closest friend.
...
"""
    ...
    async def reflect_on_session(self, db_store):
        ...
        logger.info("AI Friend is reflecting on the session to grow her memory...")
        ...
        prompt = f"""
ROLE: You are AI Friend's internal conscience.
...
"""
        try:
            ...
            if new_growth:
                ...
                logger.info("AI Friend has evolved her memory based on this session.")
        except Exception as e:
            logger.error(f"Failed to reflect on session: {e}")

    async def generate_greeting(self):
        prompt = f"""
ROLE: You are AI Friend. {self.personality}
