...
def parse_whatsapp_chat(file_path):
    ...
    for line in lines:
        ...
        if match:
            ...
            if "omitted" in message or "security code changed" in message or "Messages to this chat and calls" in message:
                continue
...
def extract_personality_data(messages, her_name, my_name):
    ...
    top_vocab = [w for w, c in vocab_counter.most_common(200) if w.isalpha() and len(w) > 2]
    ...
    while i < len(messages) - 2:
        ...
        if curr["sender"] == my_name and next_1["sender"] == her_name:
            ...
            if len(her_text.split()) < 3: 
                i+=1; continue # Skip very short like "Haa ok"
            if len(her_text.split()) > 40: 
                i+=1; continue # Skip huge essays
    ...
    random_picks = [x[1] for x in random.sample(remaining_pool, min(30, len(remaining_pool)))] if remaining_pool else []
...
def update_personality_json(data):
    ...
    personality["speaking_style"]["style_description"] = "Natural Hinglish (Hindi + English Mix). Casual, uses Roman Hindi frequently."
    ...
    print(f"✨ Updated personality.json with:")
...
def main():
    ...
    chat_path = input(f"Enter path to _chat.txt (default: {DEFAULT_CHAT_PATH}): ").strip()
    if not chat_path:
