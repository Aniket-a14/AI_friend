...
@pytest.mark.asyncio
async def test_calculate_utility_no_emotion(memory_store, mock_pool):
    pool, conn = mock_pool
    ...
    with patch.object(memory_store, 'get_embedding', return_value=[0.1]*768):
        results = await memory_store.search_memories("test query", threshold=0.1)
...
@pytest.mark.asyncio
async def test_emotional_boost(memory_store, mock_pool):
    pool, conn = mock_pool
    now = datetime.now()
    ...
    with patch.object(memory_store, 'get_embedding', return_value=[0.1]*768):
        ...
        results = await memory_store.search_memories("test query", threshold=0.1, limit=1)
        assert results[0] == "Emotional Memory"

@pytest.mark.asyncio
async def test_time_decay_ranking(memory_store, mock_pool):
    pool, conn = mock_pool
    now = datetime.now()
    ...
    with patch.object(memory_store, 'get_embedding', return_value=[0.1]*768):
        ...
        assert results[0] == "Recent memory"
