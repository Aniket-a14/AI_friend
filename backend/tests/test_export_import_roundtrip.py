...
def test_export_import_sqlite_roundtrip(tmp_path, monkeypatch):
    ...
    personality_path.write_text(json.dumps({"name": "Test Persona", "traits": ["kind"]}))
    ...
    conn.execute("CREATE TABLE identity_core (id INTEGER PRIMARY KEY, key TEXT, value TEXT)")
    conn.executemany(
    ...
    ''')
    conn.executemany("INSERT INTO agent_state (affect) VALUES (?)", [("happy",), ("curious",)])
    conn.executemany("INSERT INTO adaptive_weights (weight) VALUES (?)", [(0.5,), (0.9,)])
    conn.commit()
    conn.close()
    ...
    identity_rows = _query_sqlite_db(identity_db_path, "SELECT key, value FROM identity_core ORDER BY key")
    assert identity_rows == [("core_belief", "helpful"), ("core_memory", "birth")]
    
    state_rows = _query_sqlite_db(state_db_path, "SELECT affect FROM agent_state ORDER BY id")
    assert state_rows == [("happy",), ("curious",)]
    
    weight_rows = _query_sqlite_db(state_db_path, "SELECT weight FROM adaptive_weights ORDER BY id")
