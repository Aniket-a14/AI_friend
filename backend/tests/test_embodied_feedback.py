...
from app.contracts import AudioPlaybackProgress, AudioStop, Topics

@pytest.mark.asyncio
async def test_dialogue_truncation_on_interruption(mock_llm_service, mock_graph_db, mock_memory_store):
...
@pytest.mark.asyncio
async def test_dialogue_truncation_via_estimation_fallback(mock_llm_service, mock_graph_db, mock_memory_store):
    store = ConversationHistoryStore()
    await store.initialize()
