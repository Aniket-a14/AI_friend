...
def list_samples():
    ...
    extras = [f for f in os.listdir(SAMPLES_DIR) if f.lower().endswith((".mp3", ".mp4", ".ogg"))]
    return wavs, extras
...
def main():
    ...
    if extras:
        ...
        for e in extras:
            ...
            print(f'     ffmpeg -i "voice_samples/{e}" -ac 1 -ar 22050 "voice_samples/{wav_name}"')
        print("-" * 30)
    ...
    print("\n💡 Strategy Guide:")
    print("1. [Quick] Zero-Shot: Pick the CLEARest 5-10 second clip (Best for instant use)")
    ...
    for s in wavs:
        ...
        if not os.path.exists(os.path.join(SAMPLES_DIR, txt_file)):
            ...
            print(f"   Please create a text file named '{txt_file}' containing exactly what is said in the audio.")
