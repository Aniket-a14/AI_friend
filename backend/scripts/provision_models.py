    ...
    if not target_dir.is_dir():
        ...
        if model_path.exists():
            ...
            if sha.upper() != config["expected_sha256"].upper():
                logger.warning("🚨 Sensory Mesh integrity compromised! Re-provisioning...")
                _provision_model(config, base_dir)
            else:
...
def _provision_model(config, base_dir):
    ...
    logger.info(f"📥 Downloading {config['name']} (Expected SHA256: {config['expected_sha256'][:8]}...)")
    ...
    try:
        ...
        extracted_dir = base_dir / "models" / "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17"
