...
def print_human_readable_table(filepath):
    ...
    print("=" * 85)
    print(f"{'🧠 AI FRIEND CVS-2.0 CORE LATENCY BENCHMARKS':^85}")
    print("=" * 85)
    print(f"{'Subsystem Component':<45} | {'Mean Latency':<12} | {'p99 Tail':<10} | {'Status':<10}")
    print("-" * 85)

    for b in benchmarks:
        raw_name = b['name']
        # Clean up the name for human readers
        clean_name = raw_name.replace('test_', '').replace('_benchmark', '').replace('_', ' ').title()
        ...
        print(f"{clean_name:<45} | {mean_ms:>8.3f} ms | {p99_ms:>7.3f} ms | {status:<10}")
...
if __name__ == '__main__':
    ...
    if latest_file:
        ...
        print("No benchmark runs found. Please run `pytest tests/test_performance.py` first.")
