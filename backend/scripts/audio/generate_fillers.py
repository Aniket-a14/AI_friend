...
async def generate_fillers():
    ...
    short_ref_text = (
        "At the end of the exam, the program shows"  # Approximate text for first 4.0s
    )
    ...
    if not os.path.exists(short_ref_audio_host):
        ...
        try:
            ...
            short_ref_audio_container = (
                "output/sample_en_gold.wav"  # Fallback container path
            )
    ...
    for text in fillers:
        filename = (
            text.lower().replace("...", "").replace(" ", "_").replace(",", "") + ".wav"
        )
