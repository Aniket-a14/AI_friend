...
def get_repo_root() -> Path:
    ...
    if (current / "start.sh").exists() or (current / "docker-compose.infra.yml").exists():
        return current
...
def cmd_model(args: argparse.Namespace) -> int:
    ...
    if args.model_action == "list" or not args.model_action:
        ...
        popular = [
            ("llama3.2:3b", "Default Recommended: Fast, empathetic, 3B parameters (~2.0 GB VRAM)"),
            ("qwen2.5:3b", "High multilingual fluidity & conversational nuance (~2.0 GB VRAM)"),
            ("qwen2.5:7b", "Deep conversational depth & complex problem solving (~4.7 GB VRAM)"),
            ("deepseek-r1:7b", "High reasoning & chain-of-thought analysis (~4.7 GB VRAM)"),
            ("deepseek-r1:1.5b", "Fast chain-of-thought for lower RAM devices (~1.2 GB VRAM)"),
            ("llama3.2:1b", "Ultra-lightweight: for 8GB RAM laptops & low-power devices (~1.1 GB VRAM)"),
            ("mistral:7b", "Balanced literary eloquence & structured roleplay (~4.1 GB VRAM)"),
            ("claude-3-5-sonnet", "Cloud API Fallback (Set LLM_PROVIDER=anthropic in .env)"),
        ]
    ...
    if args.model_action == "pull":
        if not args.name:
            print("Error: Specify model name to pull (e.g. friend model pull deepseek-r1:7b)")
            return 1
...
def main() -> int:
    ...
    p_start.add_argument("--vision", action="store_true", help="Enable Moondream VLM vision agent")
    p_start.add_argument("--model", help="Select LLM model (e.g. llama3.2:3b, qwen2.5:7b, deepseek-r1:7b)")
    p_start.set_defaults(func=cmd_start)

    # model
    p_model = subparsers.add_parser("model", help="Select, pull, and inspect any local or cloud LLM model")
    p_model.add_argument("model_action", nargs="?", choices=["list", "set", "pull"], default="list", help="Action to perform")
    p_model.add_argument("name", nargs="?", help="Model name (e.g. qwen2.5:7b, deepseek-r1:7b, llama3.2:1b)")
    ...
    # status
    p_status = subparsers.add_parser("status", help="Display system status, hardware scan, and container health")
    p_status.set_defaults(func=cmd_status)

    # talk
    p_talk = subparsers.add_parser("talk", help="Start interactive conversation REPL in terminal")
    p_talk.set_defaults(func=cmd_talk)

    # persona
    p_persona = subparsers.add_parser("persona", help="Run the natural language persona authoring wizard")
    p_persona.set_defaults(func=cmd_persona)

    # voice
    p_voice = subparsers.add_parser("voice", help="Enroll an 8-second custom voice sample")
    p_voice.add_argument("--duration", type=int, default=8, help="Recording length in seconds")
    p_voice.set_defaults(func=cmd_voice)

    # backup
    p_backup = subparsers.add_parser("backup", help="Export or restore 4-store friend archives")
    ...
    b_imp.add_argument("--force", action="store_true", help="Confirm destructive table overwrite")
    b_imp.set_defaults(func=cmd_backup_import)

    # logs
    p_logs = subparsers.add_parser("logs", help="Tail live container logs")
    p_logs.add_argument("service", nargs="?", help="Specific service name (e.g. brain_agent, postgres)")
    p_logs.set_defaults(func=cmd_logs)

    # update
    p_update = subparsers.add_parser("update", help="Pull latest release updates from Git and rebuild")
