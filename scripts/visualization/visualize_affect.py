...
def plot_sample_pad_trajectory():
    ...
    pleasure = 0.5 + 0.3 * np.sin(time_points / 10) - 0.4 * np.exp(-(time_points - 30)**2 / 20)
    
    # Arousal: Spikes during the 'disagreement'
    arousal = 0.2 + 0.6 * np.exp(-(time_points - 30)**2 / 30) + 0.1 * np.random.normal(0, 0.1, 100)
    ...
    plt.title('Affective Trajectory: Tier-5 Sovereign Mesh', fontsize=14, fontweight='bold')
    ...
    plt.annotate('User Disagreement', xy=(30, 0.5), xytext=(35, 0.8),
