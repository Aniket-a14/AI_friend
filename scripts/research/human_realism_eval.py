...
def module1_computational_footprint():
    ...
    print(f"  Total CVS-3.0 Average CPU Load: {total_cpu:.2f}% (iMac Host Node)")
...
def module2_perception_knowledge():
    ...
    print(f"  CVS-3.0 Cached Traversal Depth 1-hop: {cvs_cached_latencies[0]:.3f} ms | 3-hop: {cvs_cached_latencies[2]:.3f} ms")
...
def module3_cognitive_states_endocrine():
    ...
    print(f"  Dynamic Threat Appraisal (t=2s): Cortisol spiked to {cortisol[2]:.2f} | Dopamine dropped to {dopamine[2]:.2f}")
    print(f"  Active Coping Appraisal (t=20s): Cortisol decayed to {cortisol[20]:.2f} | Dopamine rose to {dopamine[20]:.2f}")
    print(f"  Stabilized Homeostasis  (t=80s): Cortisol stabilized at {cortisol[80]:.2f} | Dopamine at {dopamine[80]:.2f}")
...
def module4_physiological_entrainment(cognitive_data):
    ...
    pleasure = np.array(cognitive_data["pleasure"])
    ...
    print(f"  Physiological Baseline (t=0s):  HR={hr[0]:.1f} BPM | RR={rr[0]:.1f} Breaths/Min | HRV={hrv[0]:.1f} ms")
    print(f"  Stressed Physiological (t=5s):  HR={hr[5]:.1f} BPM | RR={rr[5]:.1f} Breaths/Min | HRV={hrv[5]:.1f} ms (High Stress)")
    print(f"  Recovered Homeostasis (t=80s):  HR={hr[80]:.1f} BPM | RR={rr[80]:.1f} Breaths/Min | HRV={hrv[80]:.1f} ms (Relaxed)")
    ...
    print(f"  Paralinguistic Sentiment Mapping Precision (CVS-3.0): {paralinguistic_metrics['high_stress']['tag_precision']*100:.1f}%")
...
def generate_visualizations(comp_data, db_data, cog_data, phys_data):
    ...
    axes[0].text(2.5, 0.85, "Threat Injected", color="#dc3545", fontweight="bold", fontsize=8)
    axes[0].set_ylabel("Normalized Range [0.0, 1.0]", fontsize=9)
    axes[0].set_title("A: Affective & Endocrine Homeostatic Dynamics under Stressor", fontweight="bold", fontsize=10)
    axes[0].legend(loc="upper right", frameon=True, facecolor="white", framealpha=0.9)
    axes[0].set_ylim(-0.05, 1.05)
    
    # Subplot 2: Heart Rate Entrainment
    axes[1].plot(time_steps, phys_data["heart_rate_bpm"], label="Coupled Heart Rate (HR)", color="#dc3545", linewidth=2)
    axes[1].axvline(x=2, color="#dc3545", linestyle=":", alpha=0.8)
    axes[1].set_ylabel("Heart Rate (BPM)", fontsize=9)
    axes[1].set_title("B: Physiologic Entrainment: Coupled Heart Rate (HR) Response", fontweight="bold", fontsize=10)
    ...
    p1 = axes[2].plot(time_steps, phys_data["respiration_rate_breaths_min"], label="Breathing Rate (RR)", color="#007bff", linewidth=2)
    p2 = ax3_twin.plot(time_steps, phys_data["hrv_rmssd_ms"], label="Heart Rate Variability (HRV)", color="#6f42c1", linewidth=2, linestyle="--")
    ...
    axes[2].legend(plots, labels, loc="upper right", frameon=True, facecolor="white", framealpha=0.9)
    
    axes[2].set_xlabel("Elapsed Time (Seconds)", fontsize=10)
    axes[2].set_title("C: Physiologic Entrainment: Respiration & Autonomic HRV Coupling", fontweight="bold", fontsize=10)
    ...
    axes[0].set_title("Speech Turn-Taking / Barge-in Latency", fontweight="bold", fontsize=10)
    for idx, val in enumerate(values_lat):
        axes[0].text(idx, val + 40, f"{val}ms", ha="center", va="bottom", fontsize=8, fontweight="bold")
    ...
    axes[1].set_title("Theory of Mind Emotion Inference MAE", fontweight="bold", fontsize=10)
    for idx, val in enumerate(values_tom):
        axes[1].text(idx, val + 0.01, f"{val:.2f}", ha="center", va="bottom", fontsize=8, fontweight="bold")
    ...
    axes[2].set_title("Memory Retrieval Performance (Recall@5)", fontweight="bold", fontsize=10)
    for idx, val in enumerate(values_ret):
        axes[2].text(idx, val + 1.5, f"{val:.1f}%", ha="center", va="bottom", fontsize=8, fontweight="bold")
...
def main():
    print("🚀 Starting AI Friend CVS-3.0 Human Realism & Physiological Entrainment Benchmarks...")
    create_directories()
    ...
    final_json = {
        ...
        "module4_physiological_coupling": {
            "heart_rate_bpm": {
                "baseline": round(phys_mean := float(np.mean(m4_results["heart_rate_bpm"][:3])), 2),
                "peak_stress": round(float(np.max(m4_results["heart_rate_bpm"])), 2),
                "recovery_average": round(float(np.mean(m4_results["heart_rate_bpm"][60:])), 2)
            },
            "respiration_breaths_min": {
                "baseline": round(float(np.mean(m4_results["respiration_rate_breaths_min"][:3])), 2),
                "peak_stress": round(float(np.max(m4_results["respiration_rate_breaths_min"])), 2),
                "recovery_average": round(float(np.mean(m4_results["respiration_rate_breaths_min"][60:])), 2)
            },
            "hrv_rmssd_ms": {
                "baseline": round(float(np.mean(m4_results["hrv_rmssd_ms"][:3])), 2),
                "minimum_stress": round(float(np.min(m4_results["hrv_rmssd_ms"])), 2),
                "recovery_average": round(float(np.mean(m4_results["hrv_rmssd_ms"][60:])), 2)
            },
            "paralinguistics": m4_results["paralinguistics"]
        }
    }
    
    out_path = "scripts/research/human_realism_results.json"
    with open(out_path, "w") as f:
        json.dump(final_json, f, indent=2)
        
    print(f"💾 Full quantitative dataset written to: {out_path}")
    print("📊 Dynamic trajectory CSV is fully compatible with latex pgfplots or pandas.")
