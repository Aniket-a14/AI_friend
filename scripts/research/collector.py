...
async def run_collector():
    ...
    with open(LOG_FILE, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "timestamp", "elapsed_sec", "pleasure", "arousal", "dominance", 
            "trust", "cortisol", "dopamine", "fatigue", 
            "inferred_valence", "inferred_arousal", 
            "snr", "wing", "tags", "emotion"
        ])
    ...
    async def state_handler(msg):
        try:
            ...
            print(f"💓 [State] P:{p:+.2f} A:{a:+.2f} D:{d:+.2f} | T:{trust:.2f} | C:{cortisol:.2f} DP:{dopamine:.2f} F:{fatigue:.2f} | ToM V:{inferred_valence:+.2f} A:{inferred_arousal:.2f} | {emotion}")
