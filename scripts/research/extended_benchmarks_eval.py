...
def generate_publication_charts(data):
    ...
    ax.plot(angles, cvs_scores, linewidth=2, linestyle='solid', label='AI Friend CVS-3.0 (Sovereign)', color='#10b981') # Premium emerald
    ...
    axes[0].plot(turns, data["multi_turn_coherence"]["cvs_coherence"], label="CVS-3.0 (Sovereign)", color="#10b981", linewidth=2)
    ...
    rects1 = axes[1].bar(x - width/2, cvs_values, width, label="CVS-3.0 (iMac Host Node)", color="#10b981", edgecolor="black", alpha=0.85)
...
def compile_pdf_report(data):
    ...
    pdf_path = "scripts/research/CVS-3.0_Mind_Benchmarking_Report.pdf"
    ...
    story.append(Paragraph("Empirical Validation of AI Friend CVS-3.0: A Low-Latency 12-Dimensional Sovereign Mind Mesh and Autonomic Realism Architecture", title_style))
    ...
    table_data_i = [
        ...
        [Paragraph("<b>AI Friend CVS-3.0 (Ours)</b>", table_cell_bold), Paragraph("Apple iMac (Host Profiler) <br/> NVIDIA Jetson AGX Orin (Target Platform)", table_cell), Paragraph("8-16 GB / 64 GB", table_cell), Paragraph("30 W (Target Mode)", table_cell), Paragraph("Localized Sovereign NATS Mesh + Llama-3.2 1B", table_cell)],
    ...
    story.append(Paragraph("The core innovation of CVS-3.0 is the formal mathematical coupling of cognitive reasoning with emotional, paralinguistic, and physiological homeostatic parameters. Unlike unmanaged static platforms, CVS-3.0 models real-time hormones and autonomic cardiovascular indicators:", body_style))
    ...
    story.append(Paragraph("Empirical benchmarks demonstrate significant advantages for CVS-3.0 across all categories. Table II summarizes the core findings, contrasting the edge-native CVS-3.0 against standard industrial HRI orchestrations.", body_style))
    
    # Table II: Metrics Summary
    table_data_ii = [
        [Paragraph("Benchmarking Metric (N=1000)", table_header_cell), Paragraph("CVS-3.0 (Ours)", table_header_cell), Paragraph("Industry Baseline / SOTA", table_header_cell), Paragraph("Speedup / Improvement", table_header_cell), Paragraph("Academic Source", table_header_cell)],
    ...
    story.append(t2)
    story.append(Paragraph("TABLE II: COMPREHENSIVE EXPERIMENTAL BENCHMARK METRICS SUMMARY FOR CVS-3.0 COGNITIVE MIND MESH", caption_style))
    ...
    table_data_iii = [
        [Paragraph("State / Stress Context", table_header_cell), Paragraph("CVS-3.0 Realism Indicators (Ours)", table_header_cell), Paragraph("Standard HRI Baseline", table_header_cell), Paragraph("Fidelity Improvement", table_header_cell)],
...
def main():
    ...
    if os.path.exists(artifact_dir):
        import shutil
        shutil.copy(pdf_path, os.path.join(artifact_dir, "CVS-3.0_Mind_Benchmarking_Report.pdf"))
