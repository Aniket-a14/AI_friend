...
def module2_theory_of_mind():
    ...
    bp1 = axes[0].boxplot(v_data, patch_artist=True, labels=["Industry Baseline", "CVS-3.0 (Ours)"])
    axes[0].set_title("Valence Absolute Inference Error", fontweight='bold')
    axes[0].set_ylabel("Absolute Error Magnitude")
    
    bp2 = axes[1].boxplot(a_data, patch_artist=True, labels=["Industry Baseline", "CVS-3.0 (Ours)"])
    ...
    plt.tight_layout()
    plt.savefig("scripts/research/cognitive_tom_errors.png")
    plt.close()
    
    return {
        "cvs_valence": {"mae": round(cvs_v_mae, 4), "rmse": round(cvs_v_rmse, 4)},
        "cvs_arousal": {"mae": round(cvs_a_mae, 4), "rmse": round(cvs_a_rmse, 4)},
        "baseline_valence": {"mae": round(base_v_mae, 4), "rmse": round(base_v_rmse, 4)},
        "baseline_arousal": {"mae": round(base_a_mae, 4), "rmse": round(base_a_rmse, 4)}
    }
...
def module3_memory_actr():
    ...
    # Print metrics
    print(f"  CVS-3.0 (ACT-R)  Recall@1: {cvs_recall[0]*100:.1f}% | Recall@3: {cvs_recall[2]*100:.1f}% | Recall@5: {cvs_recall[4]*100:.1f}%")
    print(f"  Baseline (S-RAG) Recall@1: {base_recall[0]*100:.1f}% | Recall@3: {base_recall[2]*100:.1f}% | Recall@5: {base_recall[4]*100:.1f}%")
    
    # Plot Recall@K Curve
    plt.figure(figsize=(6, 4), dpi=300)
    plt.plot(ks, cvs_recall * 100, marker='o', color='#007bff', linewidth=2.5, label='CVS-3.0 (ACT-R Memory Search)')
    ...
    plt.tight_layout()
    plt.savefig("scripts/research/cognitive_rag_recall.png")
    plt.close()
...
def module4_conflict_resolver():
    ...
    print(f"  CVS-3.0 Conflict Resolver: F1={cvs_metrics['f1']*100:.1f}%, Mean Latency={np.mean(cvs_latencies):.1f}ms")
...
def main():
    ...
    out_path = "scripts/research/cognitive_metrics_results.json"
