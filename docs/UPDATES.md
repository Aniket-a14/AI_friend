# Executive Summary  
We recommend a **hybrid cognitive architecture** (e.g. a BDI-style agent with reactive planning layers) that aligns with your modular agents and message bus. This lets the Brain Agent use a *belief* store (Neo4j/PGVector memory), *desires* (goals/tasks) and *intentions* (plans executed via behavior trees or LLM-generated actions)【10†L130-L139】【14†L9-L17】. For decision-making, combine **behavior trees (BTs)** for fast, modular action selection【25†L54-L63】 with an LLM-based planner or goal manager to handle high-level social/emotional goals. Reinforcement (e.g. Monte Carlo search) can evaluate candidate responses by social reward functions (trust, comfort, engagement). Memory should be **hybrid**: use a vector store (PGVector/FAISS/Milvus) for semantic recall and a graph DB (Neo4j) for explicit facts and relationships. Tag each memory with metadata (importance_score, emotional_weight, last_recalled_at, certainty, source, version) and apply recency-weighted decay so that seldom-used facts fade over time【37†L354-L362】.  Internal state/emotion can be modeled in multiple dimensions (mood, arousal/energy, trust, attachment). For example, mood = mood·(1–α)+α·(event_valence) with exponential decay; trust and attachment update based on positive/negative interactions. Every response must be **validated** against the agent’s persona: run a constraint check or small “self-consistency” LLM pass to catch persona violations, regenerating outputs that conflict with core traits or facts.  

On the stack, use **local-first** components wherever possible. For voice, we suggest open-source TTS: e.g. Coqui’s XTTS or Tortoise for high-quality offline synthesis, or GPT-SoVITS (fast 1-minute voice cloning)【46†L1-L4】【45†L334-L342】. Keep ElevenLabs or a cloud option only as a fallback. For STT, OpenAI’s Whisper (or Faster Whisper) or VOSK are good offline choices. In future vision/perception, use real-time models like YOLO (object detection) and Meta’s Segment-Anything (image segmentation) and CLIP (zero-shot vision-language)【52†L53-L61】【50†L55-L64】【54†L50-L53】. The existing NATS JetStream bus is well-suited for orchestrating microservices and buffering inter-agent events【56†L95-L103】. We also recommend extensive testing: automated scenarios for persona consistency and memory retrieval (e.g. prompting contradictions) and monitoring dashboards (logs, metrics) to track memory freshness, emotional drift, and response quality. Finally, the transition plan should move stepwise from cloud LLMs to local models (e.g. LLaMA 3, Mistral 7B, Falcon, etc.) and from APIs to self-hosted inference (with 16–24GB GPUs). Below we detail each area, compare options (in tables), and include mermaid diagrams and pseudocode for clarity.
...
## 10. Migration Plan  
**Stage 1 – Local Knowledge Integration:** Swap out cloud APIs where easy. For memory/context, migrate from Supabase to local stores (Neo4j + PGVector). For persona, load existing “Pankudi” data into Neo4j nodes.  
