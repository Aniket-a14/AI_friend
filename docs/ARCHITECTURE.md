...
- **Speculative Filler Interruption Masking**: To hide compilation/synthesis latency, the system uses a **1200ms speculative pause filler threshold** (`VOICE_FILLER_THRESHOLD`), measured from when generation actually starts (after conversational pacing, not before it). 400ms is the baseline generation overhead observed before Ollama's own TTFT, so a threshold near it fired on almost every turn; 1200ms only fires once TTFT is running noticeably above that baseline. If the first audio chunk is not generated within 1200ms, a single vocal filler (e.g. *"hmm"*, *"let's see"*) is dispatched to maintain turn flow — suppressed if one already fired within `VOICE_FILLER_MIN_INTERVAL_SECONDS` (1.5s) or if a previous turn's audio is still backed up past `VOICE_FILLER_MAX_PLAYBACK_BACKLOG` (4 queued frames).
