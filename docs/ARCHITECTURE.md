...
2.  **Tier 2: Relational Episodic Memory & Subconscious HSL Hybrid Archive**:
    ...
    *   *HSL Hybrid Retrieval (Semantic + Lexical + Learned Priming)*: To bypass synonym-mismatch failures, subconscious lookup uses a true hybrid query combining HNSW semantic proximity with Porter-style root-lemma stem extraction (`_get_stem`) and **learned lexical priming** executing ILIKE keyword lookups. Cue expansion no longer reads a hardcoded thesaurus; it draws on the `MentalLexicon` (`lexicon_store.py`) — a vocabulary that boots with a small generic innate seed and then acquires words and distributional co-occurrence associations from lived conversation, persisted in the `vocabulary` / `lexical_associations` tables.
