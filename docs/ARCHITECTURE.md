...
- **Speculative Filler Interruption Masking**: To hide compilation/synthesis latency, the system uses a **250ms speculative pause filler threshold** (`VOICE_FILLER_THRESHOLD`). If the first audio chunk is not generated within 250ms, immediate vocal fillers (e.g. *"Hmm"*, *"Accha"*) are dispatched to maintain turn flow.
