...
```math
Score_i = A_i + W_{\text{spread}} \cdot \text{Similarity}_{\text{eff}} - 0.5 \cdot \text{dist\_emo}
```
...
```math
\text{ICI} = \gamma \cdot \left(1 - P_{\text{false\_trigger}}\right) \cdot \exp\left(-\frac{\left|t_{\text{stop}} - t_{\text{interject}}\right|}{\tau_{\text{overlap}}}\right)
```
...
```math
P_{\text{vocal}} = 1.0 + \tanh(0.05 \cdot V + 0.15 \cdot Ar - 0.10 \cdot D - 0.05 \cdot F + \text{dist\_pitch\_mod})
```
...
```math
V_{\text{vocal}} = 0.40 + 0.60 \cdot D + \text{dist\_vol\_mod}
```
...
```math
\Delta t_{\text{reflection}} \ge \text{Config.REFLECTION\_MIN\_INTERVAL\_SECONDS}
```
...
```math
\text{Confidence}(\text{Evolution}) \ge 0.8 \implies \text{evolve\_persona}()
```
