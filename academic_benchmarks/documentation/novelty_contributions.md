...
\begin{itemize}
    ...
    \item \textbf{Dual-Gated Turn-Taking Gating:} A dual-loop turn-taking architecture combining a System 1 fast-loop DSP audio hook (interrupting robot playback within \text{[NOT MEASURED --- no pause mechanism exists yet, see \S1.1]}) with a speculative System 2 text segmenter to distinguish true semantic user interruptions from ambient background noise.
